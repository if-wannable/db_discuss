...
async function fetchAuthor(id, offset) {
  ...
  try {
    const res = await fetch(API_BASE + "/api/author/" + encodeURIComponent(id) + "?" + buildParams(offset), {
      signal: ctrl.signal,
    });
...
function renderSummary(d) {
  ...
  $("#exportBtn").href = API_BASE + "/api/author/" + encodeURIComponent(d.author_id) + "/export" + (expQs ? "?" + expQs : "");
...
$("#authorInput").addEventListener("input", (e) => {
  ...
  suggestTimer = setTimeout(async () => {
    try {
      const res = await fetch(API_BASE + "/api/author-suggest?q=" + v);
